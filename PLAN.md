...
Mantener el modelo actual de mutaciones inmediatas, pero agregar una operación server-authoritative `discardPendingKitchenChanges` que restaure el snapshot enviado (`sentQuantity`, `sentNotes`, `sentModifiersSnapshot`) y quite `pendingCancellation`; los ítems `draft` pendientes se eliminan sin tocar tickets históricos. La interfaz decide entre salida inmediata y confirmación usando `hasPendingKitchenChanges`; al confirmar el descarte, serializa y espera la confirmación server-side de todas las mutaciones en vuelo antes de restaurar. El mutator también puede resolver la orden abierta por mesa cuando la réplica aún no expone un `orderId` o conserva uno ya reemplazado. Así se corrigen tanto la persistencia accidental como la carrera de la cola sin convertir el flujo de envío a cocina en una operación local nueva.
...
- [x] Modelar el estado de salida (`isDiscardingChanges`) en `usePosTableOrder`; marcar toda mutación de cocina, drenar la cola confirmada por servidor antes del descarte y resolver la orden por mesa cuando el `orderId` aún no está replicado o ya fue reemplazado. Limpiar refs/cola/mesa sólo después del éxito. Mantener rutas separadas para cierre/cancelación exitosos, que ya terminan la orden y no deben intentar restaurarla.
