...
function formatPaymentSummary(
	...
	if (sale.status === "cancelled") return "Venta anulada";
...
export function SalesPage() {
	...
	const listParams = useMemo(() => {
		...
		return {
			...
			status:
				(SALE_STATUS_VALUES as readonly string[]).includes(status)
			...
			balanceStatus:
				(SALE_BALANCE_STATUS_VALUES as readonly string[]).includes(
					balanceStatus,
				)
	...
	const saleDetailQuery = useSaleDetail(
		isDetailOpen ? selectedSaleId : null,
	);
			...
			<div className={isMobile ? "space-y-4" : "grid gap-4 md:grid-cols-2"}>
				...
				{isTodayView ? (
					...
					<>
						...
						<FilterField
							label="Hasta"
							htmlFor={`${idPrefix}${salesEndDateId}`}
						>
							<Input
	...
	return (
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				...
				<Badge
					className={`${getSaleStatusBadgeClass(sale.status)} px-2 py-0.5 text-xs border-0`}
				>
