...
- [x] **Shifts (list/detail)** → query `shifts.list` con filtros ZQL + cursor `{ openedAt, id }`; reutiliza `sales.filterOptions` / `sales.terminalOptions` para dropdowns; `differenceStatus` y `hasMovements=no` se refinan en cliente. Índice `(organization_id, opened_at)`. Eliminados `shifts.byOrg` y `SHIFTS_SYNC_LIMIT`.
...
- [x] **Sales (list/detail/cancel)** → queries `sales.list` / `sales.byId` con filtros en ZQL, paginación por cursor (`.start()`), `sales.filterOptions` + `sales.terminalOptions`; capa `features/sales/sales.shared.ts`; mutator server-only `sales.cancel`; hooks `useSalesList` / `useSaleDetail` / `useCancelSaleMutation`. Índice `(organization_id, created_at)` en `sale`. Eliminados `sales.byOrg` y `SALES_SYNC_LIMIT`.
- [x] Sustituir `useSalesPage` para que renderice desde Zero.

> **Ventana de sync (sales list):** `sales.list` pagina en servidor con filtros ZQL + cursor `{ createdAt, id }` (`.start()`). Cada página sincroniza `limit + 1` filas livianas (sin detalle de ítems en queries separadas — la lista incluye `items` solo para `itemCount`). `balanceStatus` se refina en cliente sobre la página actual. El total exacto solo está disponible en la última página (`hasMore === false`).
