...
const organizationImplementer =
  implement(organizationContract).$context<AppContext>();
...
function getJoinLinkErrorMessage(status: Exclude<JoinLinkStatus, "active">) {
  switch (status) {
    case "expired":
...
export const selection = authedProcedure.selection.handler(
  async ({ context }) => {
...
export const updateMemberRole = orgRequiredProcedure.updateMemberRole.handler(
  async ({ input, context }) => {
    const organizationId = context.organizationId;
    const user = context.user;
    ...
    const [targetMemberRow] = await context.db
      ...
      .where(
        and(
          eq(member.organizationId, organizationId),
          or(
            eq(member.id, input.memberIdOrEmail),
            eq(userTable.email, input.memberIdOrEmail)
          )
        )
      )
      .limit(1);
...
export const updateOrganization =
  orgRequiredProcedure.updateOrganization.handler(
    async ({ input, context }) => {
      const organizationId = context.organizationId;
      const user = context.user;
      ...
      if (input.slug !== undefined) {
        setData.slug = input.slug;
      }
      if (input.logo !== undefined) {
        setData.logo = input.logo;
      }
      ...
      return (
        updatedOrg ?? { id: organizationId, name: "", slug: "", logo: null }
      );
    }
  );

export const deleteOrganization =
  orgRequiredProcedure.deleteOrganization.handler(
    async ({ input, context }) => {
      const user = context.user;
      const organizationId = context.organizationId;
