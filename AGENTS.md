...
- Server state: TanStack Query (legacy oRPC surfaces) + Zero (`@rocicorp/zero`) for new and migrated surfaces.
- API: oRPC in OpenAPI mode (legacy) **and** Zero query/mutate endpoints (`/api/zero/*`). The two coexist while we migrate per `MIGRATION_PLAN.md`.
...
Zero is the target API for new and migrated surfaces (see `MIGRATION_PLAN.md`). It coexists with oRPC during the migration; do not remove oRPC code unless the corresponding surface has fully moved to Zero.
...
- `resolveZeroAuth` projects the better-auth session into `ZeroContext` only when the session's active organization still has a matching `member` row. The query/mutate handlers in `server/zero/handler.server.ts` always pass it as `ctx`.
...
### Coexistence with oRPC

- The Zero router is mounted at `/api/zero` **before** the oRPC `/api/*` catch-all in `server/hono.ts`. Do not invert that order.
- A given UI surface should consume Zero **or** oRPC for the same data, never both. Migrate atomically per surface and delete the corresponding oRPC router/contract in the same change.
